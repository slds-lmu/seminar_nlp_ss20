...
Table : (\#tab:complexity-operations-path-length) Minimum number of sequential operations and the maximum path length for long-range dependencies. $n$ is the sequence length, $k$ the kernel size and $d$ the representation dimension. Source: \cite{vaswani2017attention}, Table 1

As can be seen in Table \@ref(tab:complexity-operations-path-length), the Self-Attention mechanism
used in the Transformer architecture can model long-range dependencies in constant time
while also only needing a constant number of operations due to extensive parrallelization.
This, and the multiple attention heads employed in their model which learn different
dependencies, enables models with the Transformer architecture to be used in transfer learning.
