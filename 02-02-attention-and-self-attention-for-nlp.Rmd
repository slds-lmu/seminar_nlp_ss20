...
NMT. Although the architecture of the neural network proposed by Luong et al. differs,
...
Table 2.2.1 : (\#tab:luong-score-functions) Different score function proposed by Luong et al.
...
input at the same time. This also enables the modeling of long-range dependencies regardless
of the distance.
...
Table 2.2.2 : (\#tab:complexity-operations-path-length) Minimum number of sequential operations and the maximum path length for long-range dependencies. $n$ is the sequence length, $k$ the kernel size, $d$ the representation dimension and $r$ the size of the neighborhood in restricted self-attention. Source: @vaswani2017attention, Table 1

The Transformer architecture, as a minimum, only needs a constant number of sequential operations regardless of
input length $n$ due to extensive parallelization as can be seen in Table \@ref(tab:complexity-operations-path-length).
The Maximum Path Length between long-range dependencies is also constant while the RNN
again needs a linear amount of steps due to its sequential input reading.
The fast modeling of long-range dependencies and the multiple attention heads which learn different
...
### (Some) Transformers are RNNs

The Transformer architecture is very good at modeling long-range dependencies in language.
The Transformer architecture has a drawback to this advantage over RNNs/CNNs, the high computational cost
due to its quadratic complexity per layer scaling with input length $n$ as seen in Table  \@ref(tab:complexity-operations-path-length).
...
The computational costs are especially severe in very long autoregressive tasks.

In the last few years some new variations of the vanilla transformer, as described above, were published
which tried to lower the computational cost.
Some of these variations of the Transformer architecture managed to decrease the complexity
from quadratic to $O(N\sqrt{N})$ (@child2019generating) or even to $O(N\log N)$ (@kitaev2020reformer).

A newer version from @katharopoulos2020transformers uses a linear attention mechanism for
...
can be rewriten as $V_i' = \frac{\phi(Q_i)^\top \sum_{j=1}^N\phi(K_j)V_j^\top}{\phi(Q_i)^\top \sum_{j=1}^N\phi(K_j)}$.
...
because $\sum_{j=1}^N\phi(K_j)V_j^\top$ and $\sum_{j=1}^N\phi(K_j)$ can be both computed once and resused for every
row of the query @katharopoulos2020transformers. 
No finite dimensional feature map of the exponential function exists, which makes a linearization of the
...
@vaswani2017attention introduced masked-self-attention for their decoder, which is adapted for linearized attention.
...
With the linearization through kernels we get $V_i' = \frac{\phi(Q_i)^\top \sum_{j=1}^i\phi(K_j)V_j^\top}{\phi(Q_i)^\top \sum_{j=1}^i\phi(K_j)}$.
...
$$
where  $S_i$ and $Z_i$ can be computed from $S_{i-1}$ and $Z_{i-1}$ which allows the linearized attention
with masking to scale linearly with respect to the sequence length. The derivation of
the numerator as cumulitve sums allows for the computation in linear time and constant memory,
which leads to computational complexity of $O(NCM)$ and memory $O(N \max{(C,d_k)})$ where $C$ is the dimensionalty of
...
This shows that we can rewrite the Transformer layers into RNN layers, for all similarity functions
that can be represented with $\phi$. 




- Add links and examples for Bahdanau and Luong attention mechanisms