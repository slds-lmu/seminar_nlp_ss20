...
Let's say we want to predict the next word (or a few words) in the sentence `"What time is it _"`. To do this, we feed individual words of the sequence to a RNN *one at time* (1 word = 1 layer). In every step it will combine information from the new word and all words that precede it and produce an output. After all elements were fed to RNN and encoded, the final output will be passed to the feed-forward layer which will classify it and produce the most probable word or symbol based on all previous information. In our example, it could be `"now"` or `"in Germany"` based on the previous context.
