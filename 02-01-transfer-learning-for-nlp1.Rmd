...
[I will adapt the pictures soon.]
...
A first enhancement "wave" began in 2018, where the transfer learning improved several fields in NLP, as the model layers have not to be trained from scratch. Also, Word2Vec, which was explained earlier ([chapter??]), is one example of transfer learning.
...
[Pan and Young (2010)] divided **transfer learning** in different subfields

On the one hand, there is **transductive** transfer learning where the source and target task are the same. Within transductive transfer learning there is another distinction between domain adoption (data from different domains) and cross-lingual-learning (data from different languages).[ruder thesis]
...
## Most popular models
In the following sections, the models **ELMo**, **ULMFiT**, and **GPT** are presented, which shaped the "first wave" of transfer learning before transformers like BERT developed and became popular. All models were presented in 2018 and are built in the framework of inductive sequential transfer learning. 
...
In contrast to previous word embeddings word, the representations of ELMo are functions of the whole input sentence. These representations are calculated on top of a biLMs (bidirectional language model) with two-layer character convolutions (1) and are a linear combination of the internal network states (2). This linear combination of the internal states is feature extraction of the frozen pre-trained biLM. 
<!-- [xxxxx talk with Marinana what she has in her chapter] -->

#### Pre-training: Bidirectional language model (biLM)
...
\end{aligned}
$$
<!-- [xxxxxxx check notation in the picture!!] -->
...
```
[definitely new graph!]


#### ELMo representation

After the biLM is trained on Wikipedia-103 dataset the model is frozen and the task-specific ELMo representations are calculated. 
...
For every task (question answering, sentiment analysis, etc.) the ELMo representation needs a task-specific calculation.

To sum up, ELMo is a huge enhancement compared to former word embedding, as it takes also the context into account. Additionally, it improves the relative error by 6 - 20 %. 
...
To improve the optimization in the AWD-LSTM further, [Merity ()]introduced the **Non-monotonically Triggered Average SGD** (or NT-ASGD), which is a new variant of Average Stochastic Gradient Descent (ASDG). 
...
In addition to the enhancements above the AWD-LSTM, the authors of the paper propose several other regularization and data efficiency methods: Variable Length Backpropaation Sequences (BPTT), Variational Dropout, Embedding Dropout, Reduction in Embedding Size, Activation Regularization, Temporal Activation Regularization. For further information, the paper [(merity)] is a great way to start. [Mehr hier???]
...
All in all, ULMFiT is outstanding language model which combines fine-tuning and the LSTM in a great way. By many different approaches the catastrophic forgetting is prevented and the performance increased. 
