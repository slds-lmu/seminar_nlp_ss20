...
@peter2018 have chosen a two layer bidirectional LSTM which is shown in the figure **[?????]**. As with every layer two additional context-dependent representations are added, A two-layer bidirectional model has five representations per token ($2L+1$).
...
After the biLM is pretrained, the model can be adapted to the target task. As ELMo 
...
the model is frozen and the task-specific ELMo representations are calculated. 
